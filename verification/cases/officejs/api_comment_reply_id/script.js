...
await Excel.run(async (context) => {
  ...
  let output;
  const reply = first.replies.add("reply"); reply.load("id"); await context.sync(); const found = comments.getItemByReplyId(reply.id); found.load("content"); await context.sync(); output = found.content;
